...
| Duplicate capture recovery | one lock-owned bounded recovery claim |
| Recognizer failure | one bounded stream restart without replaying old audio |
| Recognizer falls behind | ordered 2.56-second PCM buffer; overflow closes rather than dropping silently |
...
| Pixel capture | Core Audio process tap only; no ScreenCaptureKit import or screen permission |
| Said hears itself | own process excluded from the Core Audio tap description |
